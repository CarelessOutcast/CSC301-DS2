...
public class RSequentialSearchST<Key extends Comparable<Key>, Value> {
    ...
    public void put(Key key, Value val) {
        ...
        // Implying that key is somewhere in middle
        first = put(first, key, val);
    }
    
    // Put helper function
    private Node put(Node front, Key key, Value val)
    {
    	// If key exists; Replace key value
    	if (key.equals(front.key))
    	{
    		front.val = val;
    		return front;
    	}
    	// If you're on last node + key comes afterwards; Insert it at end
    	if (front.next == null && key.compareTo(front.key) > 0)
    	{
    		Node temp = new Node(key,val,front.next);
    		n++;
    		front.next = temp;
    		return front;
    	}
    	// If it wasn't the previous case ^; This implies you're in middle
    	// Check the next node to see if you need to insert it before it
      	if (key.compareTo(front.next.key) < 0)
    	{
    		Node temp2 = new Node(key,val,front.next);
    		n++;
    		front.next = temp2;
    		return front;
    	}
    	Node temp = put(front.next, key, val);
    	// relinking all the nodes
    	front.next = temp;
    	return front;
    }
    ...
    private Node delete(Node front, Key key) {
    	// TODO
    	...
    	// If you've already passed where it should be;
    	// Terminate the recursion.
    	if (key.compareTo(front.key) < 0)
    		return front;
        front.next = delete(front.next, key);
        return front;
    }
}
